...
class Builder
{
        ...
        foreach ($st_annotations as $class => $methods) {
            foreach ($methods as $name => $docs) {
                ...
                $tr = array(
                    ...
                    '{{ button }}'                  => $this->generateBadgeForButton($docs),
    ...
    protected function generateBadgeForMethod($data)
    {
        ...
        return '<span class="pull-right label '.$st_labels[$method].'">'.$method.'</span>';
    }

    protected function generateBadgeForButton($data)
    {
        $button = !empty($data['ApiMethod'][0]['button'])?$data['ApiMethod'][0]['button']:"";

        return '<span class="label label-info">'.$button.'</span>';
    }
...
<div class="panel panel-default">
    <div class="panel-heading">
        <h4 class="panel-title">
            <a data-toggle="collapse" data-parent="#accordion{{ elt_id }}" href="#collapseOne{{ elt_id }}"> 
            {{ button }}
            {{ route }}
            {{ method }} 
            </a>
